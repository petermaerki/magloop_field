"""Section builders used by legacy antenna page rendering."""
...
def load_html_fragments(
    ...
    """Load and rewrite optional HTML fragment files listed on antenna data."""
    ...
    for rel_path in rel_paths:
        ...
        try:
            ...
            fragment = rewrite_local_links_in_html_fragment(
                fragment,
...
def generate_inductivity_section(
    ...
    if inductance_file.exists():
        try:
            ...
            inductivity_text_html = rewrite_local_links_in_html_fragment(
                inductivity_text_html,
    ...
    try:
        picture_rel_paths = tuple(getattr(antenna_data, "inductivity_pictures", ()) or ())
        if picture_rel_paths:
            ...
            if image_tags:
                inductivity_pictures_html = (
                    '<div class="inductivity-pictures">' + "".join(image_tags) + "</div>"
                )
    except Exception as exc:  # pragma: no cover - best effort for optional section
        print(
    ...
    if (
        ...
        for measured in (l_100p_h_value, l_560p_h_value):
            if measured is not None and l_h_geometry_value not in (None, 0):
                deviations.append((measured - l_h_geometry_value) / l_h_geometry_value * 100.0)
